...
int main()
{
	...
	while (window.isOpen())
	{
		...
		if (nav == 1) {
			//######################## Movement ###############################
				//1-tank
			if (Keyboard::isKeyPressed(Keyboard::Right)&& s1.getPosition().x< 1140){
				s1.move(7, 0);
			}
			if (Keyboard::isKeyPressed(Keyboard::Left) && s1.getPosition().x >0){
				s1.move(-7, 0);
			}
			//2-bullet
			if (Keyboard::isKeyPressed(Keyboard::Space) && fired == 0) {
				fired = 1;
				s2.setPosition(s1.getPosition().x + 120 / 2.0, 530);
				shoot.play();
			}
